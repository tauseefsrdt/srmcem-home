...
const ClubItem = ({ name }: { name: string }) => (
  <div className="group flex items-center gap-2.5 px-[18px] py-3.5 bg-white/70 backdrop-blur-glass border border-[rgba(15,47,87,0.10)] rounded-[10px] shadow-card cursor-default transition-all duration-300 hover:-translate-y-0.5 hover:bg-white hover:border-navy-mid/30 hover:shadow-hover">
    ...
    <span className="text-[13px] font-bold text-navy-dark transition-colors duration-300 group-hover:text-gold-dark">{name}</span>
  </div>
);
  ...
  return (
    <section className={section} id="clubs">
      ...
      <div className="grid grid-cols-2 gap-[72px] mt-2 max-[900px]:grid-cols-1 max-[900px]:gap-12 reveal">
        <div>
          <div className="text-navy-mid font-bold text-[13px] tracking-[2px] uppercase mb-6">Cultural &amp; Lifestyle</div>
          <div className="grid grid-cols-2 gap-3 max-[480px]:grid-cols-1">
            {culturalClubs.map((club, i) => <ClubItem key={i} name={club} />)}
          </div>
        </div>
        <div>
          <div className="text-navy-mid font-bold text-[13px] tracking-[2px] uppercase mb-6">Technical &amp; Social</div>
          <div className="grid grid-cols-2 gap-3 max-[480px]:grid-cols-1">
